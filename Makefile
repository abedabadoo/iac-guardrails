...
scan:
	conftest test fixtures/pass/secure_plan.json -p policies/

# Confirm the insecure fixture is correctly blocked (negative test).
fail-check:
	@if conftest test fixtures/fail/insecure_plan.json -p policies/; then \
...
report:
	@conftest test fixtures/fail/insecure_plan.json -p policies/ --output json > /tmp/guardrails.json || true
